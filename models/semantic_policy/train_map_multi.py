...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return_dict = {'map_learned': map_learned_reconst, 'map_gt': map_gt_reconst,
                       'mask_grid': map_learned_mask, 'map_gt_obs': map_gt[0]}
        return return_dict
            ...
            if step % args.eval_freq == 0 and step > 0:
                ...
                with torch.no_grad():
                    for e_step, eval_batched in enumerate(dataloader_eval):
                        batch_size_cur = 1
                        map_learned = eval_batched['map_learned'].to(
                            device=device)
                        ...
                        y = argmax - 8 * x
                        map_gt = eval_batched['map_gt'].to(
                            device=device).view(batch_size_cur, 73, -1)
                        argmax_gt = torch.argmax(map_gt, dim=2)
                        argmax_gt = argmax_gt.view(-1)
                        xg = (argmax_gt/8.0).int().float()
                        yg = argmax_gt - 8 * xg
                        dist += torch.sqrt(torch.dot((x-xg),
                                           (x-xg).T) + torch.dot((y-yg), (y-yg).T))
                        loss = 0.0
                        ...
                        if e_step % 5 == 0:
                            for k in range(73):
                                if torch.sum(eval_batched['map_gt'].squeeze()[k]) == 0:
                                    ...
                                    cur_dir = 'models/semantic_policy/data/map_vis/vis_seed' + str(args.seed) + '_lr' + str(
                                    ...
                                    save = cv2.resize(pred_probs[0, k].view(
                                        8, 8).cpu().numpy(), (240, 240))
                                    save = save / np.max(save)
                                    save = (255*save).astype(np.uint8)
                                    cv2.imwrite(
                                        cur_dir + 'pred_probs.png', save)
                                    pickle.dump(pred_probs[0, k].view(8, 8).cpu(
                                    ).numpy(), open(cur_dir + "pred_probs.p", "wb"))

                                    save = cv2.resize(map_gt[0, k].view(
                                        8, 8).cpu().numpy(), (240, 240))
                                    save = save / np.max(save)
                                    save = (255*save).astype(np.uint8)
                                    cv2.imwrite(cur_dir + 'gt_probs.png', save)
                                    pickle.dump(map_gt[0, k].view(8, 8).cpu().numpy(), open(
                                        cur_dir + "gt_probs.p", "wb"))

                    dist = torch.sum(dist).detach(
                    ).cpu().item() / len(dataset_eval)
                    ...
                    torch.save(model_cop.state_dict(), 'map_pred_models_multi/' + str(args.dn) + 'seed_' + str(
