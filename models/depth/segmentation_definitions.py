...
def _compute_object_intid_to_color(object_intid: int) -> Tuple[int, int, int]:
    # Backup and restore random number generator state so as not to mess with the rest of the project
    # (e.g. if a random seed is fixed, we want to keep it fixed. If it isn't we don't want to fix it.)
    int_hash = int.from_bytes(hashlib.md5(
        str(object_intid).encode()).digest(), byteorder=sys.byteorder)
    r = 20 + int_hash % 200
...
OBJECT_INTID_TO_COLOR = {i: _compute_object_intid_to_color(
    i) for i in range(len(OBJECT_CLASSES))}
...
INTERACTIVE_OBJECT_IDS = [object_string_to_intid(
    o) for o in _INTERACTIVE_OBJECTS]
STRUCTURAL_OBJECT_IDS = [object_string_to_intid(
    o) for o in _STRUCTURAL_OBJECTS]
